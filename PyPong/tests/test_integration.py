...
class TestPongGameInitialization:
    ...
    def test_pong_game_creation(self, mock_pygame):
        ...
        with patch("PyPong.pong.Settings") as mock_settings, patch("PyPong.pong.GameStateManager") as mock_gsm, patch(
            "PyPong.pong.AudioManager"
        ) as mock_audio, patch("PyPong.pong.StatsManager") as mock_stats, patch(
            "PyPong.pong.Tournament"
        ) as mock_tournament, patch(
            "PyPong.pong.GamepadManager"
        ) as mock_gamepad, patch(
            "PyPong.pong.mobile_module"
        ) as mock_mobile, patch(
            "PyPong.pong.SettingsMenu"
        ) as mock_menu, patch(
            "PyPong.pong.get_theme"
        ) as mock_theme:
            # Setup mobile mocks
