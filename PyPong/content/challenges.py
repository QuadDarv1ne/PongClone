...
class Challenge:
    ...
    def from_dict(data: Dict[str, Any]) -> "Challenge":
        ...
        challenge = Challenge(
            data["id"], data["name"], data["description"], data["type"], data["target"], data["reward"], expires_at
        )
...
class ChallengeManager:
    ...
    def _create_challenges(self) -> None:
        ...
        self.weekly = [
            Challenge(
                f"weekly_{i}_{now.strftime('%Y%m')}", name, desc, "weekly", target * 3, reward * 2, weekly_expires
            )
    ...
    def save_challenges(self) -> None:
        ...
        try:
            ...
            data = {"daily": [c.to_dict() for c in self.daily], "weekly": [c.to_dict() for c in self.weekly]}
