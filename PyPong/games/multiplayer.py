...
from PyPong.systems.multiplayer import (
    MultiplayerBase, LocalPVP, NetworkHost, NetworkClient,
    GameState, ConnectionType, create_multiplayer
)
...
class MultiplayerMode(GameMode):
    ...
    def update(self, dt: float):
        ...
        if isinstance(self.multiplayer, NetworkHost):
            ...
            if player_num == 1:
                input_data = {"up": self.input_state["up1"], 
                             "down": self.input_state["down1"]}
            else:
                input_data = {"up": self.input_state["up2"], 
                             "down": self.input_state["down2"]}
