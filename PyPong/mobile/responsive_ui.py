...
class ResponsiveLayout:
    ...
    def update_screen_size(self, width: int, height: int) -> None:
        ...
        self._update_device_info()
        logger.info(f"Screen updated: {width}x{height}, device={self._device_type.name}, orientation={self._orientation.name}")
    ...
    def get_safe_area(self, padding_percent: float = 0.05) -> pygame.Rect:
        ...
        return pygame.Rect(
            padding_x,
            padding_y,
            self.current_width - 2 * padding_x,
            self.current_height - 2 * padding_y
        )
...
class AdaptiveText:
    ...
    def _calculate_position(self) -> Tuple[int, int]:
        ...
        return (
            self.layout.scale(self.base_x),
            self.layout.scale(self.base_y)
        )
