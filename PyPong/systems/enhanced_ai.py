...
from PyPong.core.constants import Balance, Difficulty
...
class TrajectoryPredictor:
    ...
    def predict_impact_point(
        self, ball_pos: Tuple[float, float], ball_velocity: Tuple[float, float], paddle_x: float, max_bounces: int = 3
    ) -> Optional[float]:
        """
        ...
        if (paddle_x < self.window_width // 2 and vx > 0) or (paddle_x > self.window_width // 2 and vx < 0):
            return None
    ...
    def get_trajectory_points(
        self, ball_pos: Tuple[float, float], ball_velocity: Tuple[float, float], steps: int = 50
    ) -> List[Tuple[int, int]]:
...
class EnhancedAI:
    ...
    def __init__(self, difficulty: Difficulty = Difficulty.MEDIUM, window_width: int = 1024, window_height: int = 720):
    ...
    def decide_move(self, paddle, ball, current_time: int) -> Tuple[bool, bool]:
        """
        ...
        predicted_y = self.predictor.predict_impact_point(
            (ball.rect.centerx, ball.rect.centery), (ball.velocity_x, ball.velocity_y), paddle.rect.centerx
        )
    ...
    def visualize_prediction(
        self, screen: pygame.Surface, ball, paddle, color: Tuple[int, int, int] = (255, 255, 0)
    ) -> None:
        """Draw prediction visualization (debug)"""
        if self.target_y is not None:
            # Draw target point
            pygame.draw.circle(screen, color, (int(paddle.rect.centerx), int(self.target_y)), 5)

        # Draw trajectory
        points = self.predictor.get_trajectory_points(
            (ball.rect.centerx, ball.rect.centery), (ball.velocity_x, ball.velocity_y), steps=30
        )
...
class AIPersonality:
    ...
    def __init__(self, name: str, aggression: float = 0.5, accuracy: float = 0.5, reaction_speed: float = 0.5):
        self.name = name
