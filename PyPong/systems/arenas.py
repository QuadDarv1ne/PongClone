...
class Portal:
    ...
    def draw(self, screen: pygame.Surface) -> None:
        """Draw portal"""
        # Entrance
        pygame.draw.circle(screen, self.color, self.entrance.center, self.entrance.width // 2)
        pygame.draw.circle(screen, Colors.WHITE.to_tuple(), self.entrance.center, self.entrance.width // 2, 2)

        # Exit
        pygame.draw.circle(screen, self.color, self.exit.center, self.exit.width // 2)
        pygame.draw.circle(screen, Colors.WHITE.to_tuple(), self.exit.center, self.exit.width // 2, 2)
...
class Arena:
    ...
    def _create_static_obstacles(self) -> None:
        ...
        for i in range(3):
            y = 200 + i * 150
            obstacle = Obstacle(
                pygame.Rect(self.width // 2 - 25, y, 50, 80), Colors.GRAY.to_tuple(), destructible=False
            )
        ...
        for side in [200, self.width - 250]:
            for i in range(2):
                y = 150 + i * 300
                obstacle = Obstacle(pygame.Rect(side, y, 50, 100), Colors.GRAY.to_tuple(), destructible=True, health=3)
                self.obstacles.append(obstacle)
    ...
    def _create_moving_obstacles(self) -> None:
        ...
        for i in range(4):
            ...
            obstacle = Obstacle(
                pygame.Rect(x, y, 60, 60), Colors.ORANGE.to_tuple(), destructible=False, moving=True, velocity=(vx, vy)
            )
            self.obstacles.append(obstacle)
    ...
    def _create_portals(self) -> None:
        ...
        for color in colors:
            entrance = pygame.Rect(randint(150, 300), randint(150, self.height - 150), 40, 40)
            exit_portal = pygame.Rect(
                randint(self.width - 300, self.width - 150), randint(150, self.height - 150), 40, 40
            )
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        if self.type == ArenaType.SHRINKING and self.shrink_amount > 0:
            ...
            pygame.draw.rect(screen, Colors.RED.to_tuple(), (0, 0, self.width, self.shrink_amount), 0)
            pygame.draw.rect(
                screen, Colors.RED.to_tuple(), (0, self.height - self.shrink_amount, self.width, self.shrink_amount), 0
            )
    ...
    def check_shrink_zone(self, ball) -> bool:
        ...
        if self.type == ArenaType.SHRINKING:
            if ball.rect.top < self.shrink_amount or ball.rect.bottom > self.height - self.shrink_amount:
                return True
        return False
    ...
    def get_safe_spawn_position(self) -> Tuple[int, int]:
        ...
        for _ in range(max_attempts):
            ...
            y = randint(100 + self.shrink_amount, self.height - 100 - self.shrink_amount)
