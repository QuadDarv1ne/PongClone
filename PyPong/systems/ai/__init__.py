...
class AIBase(ABC):
    ...
    @abstractmethod
    def get_target_y(self, ball_x: float, ball_y: float, ball_vx: float, ball_vy: float) -> float:
    ...
    def update(self, ball: Ball) -> Optional[bool]:
        ...
        target_y = self.get_target_y(ball.rect.centerx, ball.rect.centery, ball.velocity_x, ball.velocity_y)
...
class PredictiveAI(AIBase):
    ...
    def get_target_y(self, ball_x: float, ball_y: float, ball_vx: float, ball_vy: float) -> float:
        ...
        moving_toward = self.paddle.player_number == 1 and ball_vx < 0 or self.paddle.player_number == 2 and ball_vx > 0
        ...
        from PyPong.core.config import BALL_SIZE, WINDOW_HEIGHT
