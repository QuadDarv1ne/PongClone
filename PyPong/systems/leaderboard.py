...
class Leaderboard:
        ...
        try:
            ...
            for mode, scores in data.get("scores", {}).items():
                if mode in self.scores:
                    self.scores[mode] = [HighScore.from_dict(s) for s in scores[: self.max_entries]]
    ...
    def save(self):
        ...
        data = {
            "scores": {mode: [s.to_dict() for s in scores] for mode, scores in self.scores.items()},
    ...
    def add_score(
        self, name: str, score: int, mode: str, difficulty: str = "Medium", duration: int = 0
    ) -> Optional[int]:
        """
        Add a new score.
