...
class OptimizedRenderer:
    ...
    def __init__(
        ...
        self.dirty_renderer = DirtyRectRenderer(
            screen, game_surface, theme, settings
        )
    ...
    def render_game_optimized(self, state_manager: Any, shake: Any) -> None:
        ...
        if self.trails:
            dirty_rects = self.dirty_renderer.render_sprites_optimized(
                self.trails, background
            )
        
        if self.all_sprites:
            dirty_rects = self.dirty_renderer.render_sprites_optimized(
                self.all_sprites, background
            )
        
        if self.powerups:
            dirty_rects = self.dirty_renderer.render_sprites_optimized(
                self.powerups, background
            )
