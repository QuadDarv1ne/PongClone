...
class Ball(pygame.sprite.Sprite):
    ...
    def set_speed(self, speed: float) -> None:
        """Установить скорость мяча с сохранением направления"""
...
class PowerUp(pygame.sprite.Sprite):
    ...
    def apply_to_ball(self, ball: Ball) -> None:
        """Применить эффект к мячу"""
        if self.type == "slow_ball":
            # Замедляем мяч до 50% текущей скорости, но не меньше начальной
            slow_factor = 0.5
            new_speed = max(BALL_INITIAL_SPEED, ball.speed * slow_factor)
            ball.set_speed(new_speed)
        elif self.type == "multi_ball":
            pass  # Обработка в game logic
