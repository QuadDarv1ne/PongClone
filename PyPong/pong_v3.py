...
# Game constants
PADDLE_SPEED = 10
AI_PADDLE_SPEED = 6  # Initial AI paddle speed
...
# Difficulty settings
DIFFICULTY_LEVELS = {
    "Easy": {"ai_paddle_speed": 4, "ball_speed_increase": 1.05},
    "Medium": {"ai_paddle_speed": 6, "ball_speed_increase": 1.1},
    "Hard": {"ai_paddle_speed": 8, "ball_speed_increase": 1.2},
}
...
def show_menu():
    ...
    difficulty_text = menu_font.render("Press 1 for Easy, 2 for Medium, 3 for Hard", True, WHITE, BLACK)
    score_text = menu_font.render(f"First to {WINNING_SCORE} points wins", True, WHITE, BLACK)
    ...
    main_surface.blit(difficulty_text, difficulty_rect)
    main_surface.blit(score_text, score_rect)
    pygame.display.update()
...
def set_difficulty(difficulty):
    global AI_PADDLE_SPEED, BALL_SPEED_INCREASE
    AI_PADDLE_SPEED = DIFFICULTY_LEVELS[difficulty]["ai_paddle_speed"]
    BALL_SPEED_INCREASE = DIFFICULTY_LEVELS[difficulty]["ball_speed_increase"]

def main():
    global paddle1, paddle2, ball
    global UP1, DOWN1, UP2, DOWN2
    paddle1 = Paddle(1, color=GREEN)
    paddle2 = Paddle(2, is_ai=True, color=YELLOW)  # Set paddle2 as AI
    ball = Ball()
    ...
    while True:
        ...
        if player1_score == WINNING_SCORE or player2_score == WINNING_SCORE:
            show_game_over(1 if player1_score == WINNING_SCORE else 2)
            return
